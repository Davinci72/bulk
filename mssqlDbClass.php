...
class mssqlDatabase {
  ...
  function __construct() {
	...
	public function open_connection() {
		...
		if (!$this->connection) {
			die("Database connection failed: " . mssql_error());
		} else {
  ...
  public function affected_rows() {
	...
	private function confirm_query($result) {
		if (!$result) {
	    $output = "Database query failed: " . mssql_error() . "<br /><br />";
	    //$output .= "Last SQL query: " . $this->last_query;
	    die( $output );
		}
	}
public function pushError($error){
   echo '<div style="padding:10px;"><span class="label label-danger error-h" data-dismiss="alert">'.$error.'</span></div>';
   }
